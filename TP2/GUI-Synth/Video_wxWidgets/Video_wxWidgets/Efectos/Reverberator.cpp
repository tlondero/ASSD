...
std::vector<double> Reverberator::applyEffect(std::vector<double> input, double T, double mix)
{
	...
	std::vector<double> M_parallel = { 4799, 4999, 5399, 5801, 6203 };
	...
	for (int i = 0; i < M_parallel.size(); i++) {
		g_parallel.push_back(-1 * pow(10, (-3 * M_parallel[i]) / (T * SAMPLE_RATE)));
	}
