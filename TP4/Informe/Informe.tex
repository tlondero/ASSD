...
Se analizaron diversas formas de lograr el seguimiento de un objeto. Se decidió volcarse por el uso de una técnica de detección del flujo óptico\footnote{Patrón de movimiento aparente entre objetos en una escena. Esta técnica extrae la información tanto de elementos estáticos como en movimiento\cite{ref:optic-flow}.} de Lucas-Kanade junto con el uso del filtro de Kalman.
La primer técnica consta del uso de información obtenida a partir de la intensidad del gradiente espacial para buscar la posición que mejor se acomoda a una imagen en movimiento \cite{ref:lucas-kanade}.

%Podría poner más de LUCAS KANADE

Por su lado, la segunda técnica consiste en un algoritmo que elabora una estimación de unas variables deseadas, mediante el uso de una serie de observaciones a lo largo del tiempo. Este filtro considera ciertas imprecisiones en las mediciones, como lo puede ser el ruido existente. 
\textcolor{orange}{Escribí una brevísima descripción de lo que se va a hacer. Profundizar y escribir el objetivo final. Justificar elección.}
...
Luego se buscó utilizar el código previamente mencionado para estimar el movimiento de un objeto en tiempo real. Es así que se realizó un soSftware que permite al usuario seleccionar un área rectangular de su cámara, siendo los elementos dentro de dicha sección los cuales son seguidos por el programa.
